...
def distance_matrix(points1, points2, norm=2, max_dist=None, dist='euclidean'):
    ...
    if dist == 'euclidean':
        ...
        dist_mat = np.linalg.norm(diff, axis=-1, ord=norm)
    elif dist == 'cosine':
        points1_normed = points1 / np.linalg.norm(points1, axis=1).reshape((-1, 1))
        points2_normed = points2 / np.linalg.norm(points2, axis=1).reshape((-1, 1))
        dist_mat = points1_normed @ points2_normed.T
